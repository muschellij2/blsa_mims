...
dat_i <- dat_acc %>% filter(subj_id == subj_id_i) %>% as.data.frame()
dat_i$y <- dat_i %>% pull(all_of(var_tmp)) 
dat_i <- dat_i %>% mutate(y = replace(y, wear_and_valid_flag == 0, NA))
date_vec <- unique(dat_i$HEADER_TIME_STAMP_date)
date_vec_l <- length(date_vec)

Lt <- lapply(1 : date_vec_l, function(val) return(1 : 1440))
Ly <- vector("list", length = date_vec_l)
for (j in 1 : date_vec_l){
  Ly[[j]] <- dat_i %>% filter(HEADER_TIME_STAMP_date == date_vec[j]) %>% pull(y)
}
FPCA_out <- FPCA(Ly, Lt)

# refund::fpca.face on entire population (all this is doing is smoothing the data)
...
fit_MIMS <- gam(MIMS ~ s(AC, bs = "cr", k = k_indep), data = dat_acc)
...
# fit GAM model to all data assuming independence 
fit_df <- dat_acc %>% filter(subj_id_F %in% subj_id_vec_F[1:20])
dim(fit_df)
fit_MIMS <- gam(MIMS ~ s(AC, bs = "cr", k = k_indep), 
                data = fit_df
                )
summary(fit_MIMS)
plot(fit_MIMS, shade = TRUE, shade.col = "palegreen", bty = "l")


# ------------------------------------------------------------------------------

t1 <- Sys.time()
fit_MIMS_gamm_n10 <- gamm(MIMS ~ s(AC, bs = "cr", k = 10), 
                          data = dat_acc %>% filter(subj_id_F %in% subj_id_vec_F[1:10]),
                          random = list(subj_id_F = ~ 0 + MIMS_resid_FPC1)
)
t2 <- Sys.time()
t2 - t1
# n=10
# Time difference of 18.58492 mins


t1 <- Sys.time()
fit_MIMS_gamm <- gamm(MIMS ~ s(AC, bs = "cr", k = 10), 
...
t2 - t1
# n=20
# Time difference of 35.4154 mins
...
# 
t1 <- Sys.time()
# should have been fit_MIMS_gamm_30
fit_MIMS_gamm_n5 <- gamm(MIMS ~ s(AC, bs = "cr", k = 10), 
                          data = dat_acc %>% filter(subj_id_F %in% subj_id_vec_F[1:30]),
                          random = list(subj_id_F = ~ 0 + MIMS_resid_FPC1)
)
t2 <- Sys.time()
t2 - t1
