...
mastervisit <- 
  ...
  filter(!is.na(BMI)) %>%
  filter(!is.na(DOV)) %>%
  rename(idno_mastervisit = IDNo, visit_mastervisit = Visit) %>%
  mutate(entry_mastervisit = 1)  %>%
  distinct()
dim(mastervisit)

mastervisit_meta_fpath <- paste0(here::here(), "/covariates/mastervisit_meta.txt")

# Gait speed
# 
# 29        UGSpeed                     PERF: usual gait speed in meters per second

# Perceived faitgability
# ???

# Expanded SPPB Score 
# 
# 37        HABCPPB         PERF: expanded short physical performance battery score

# Endurance walking
# 57       C400Secs                                LDCW: 400 meter walk - # Seconds
