...
acc_corr_fpath <- paste0(here::here(), "/results/2021-02-25-correlations_between_measures.rds")
acc_corr <- readRDS(acc_corr_fpath)
dim(acc_corr)
# [1] 1442   15

masterfile_path <- paste0(here::here(), "/data_processed/2021-02-25-measures_masterfile_winsorized.rds")
masterfile <- readRDS(masterfile_path) %>% as.data.frame()
dim(masterfile)
# [1] 6147240      10

masterfile_uw_path <- paste0(here::here(), "/data_processed/2021-02-25-measures_masterfile.rds")
...
for (i in 1 : nrow(corr_subj_id_sub)){ # i <- 1
  ...
  plt_df_i   <- masterfile %>% filter(subj_id == subj_id_i) %>% filter(wear_flag == 1)
...
# ------------------------------------------------------------------------------
# (1) investigate closely the case of ID = 5997

subj_id_tmp <- 5997

# windsorized
masterfile_sub <- masterfile %>% filter(subj_id == subj_id_tmp)
tmp <- masterfile_sub %>% filter(ENMO > 4) %>% select(-c(file_id, visit_id))
tmp

# non-windsorized
masterfile_uw_sub <- masterfile_uw %>% filter(subj_id == subj_id_tmp)
tmp <- masterfile_uw_sub %>% filter(ENMO > 4) %>% select(-c(file_id, visit_id))
tmp
...
# read accelerometry measures master file
dat_corr_fpath <- paste0(here::here(), "/results/2021-02-25-correlations_between_measures.rds")
dat_corr <- readRDS(dat_corr_fpath)

sort(dat_corr$AC_ENMO) %>% head(100)
