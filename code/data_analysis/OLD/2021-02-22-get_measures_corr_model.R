...
acc_corr_fpath <- paste0(here::here(), "/results/2021-01-19-correlations_between_measures.rds")
...
for (i in 1:4){ # i <- 3
  ...
  plot_obj_i <- (plot(fit_i, shift = 0, seWithMean = TRUE))[[1]]
  ...
  shift_bmi   <- mean(dat$bmi * coef(fit_tmp)[2])
  shift_sex   <- mean((dat$sex == "Male") * coef(fit_tmp)[3])
