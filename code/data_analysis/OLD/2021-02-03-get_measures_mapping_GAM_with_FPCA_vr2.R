...
fit_MIMS <- gam(MIMS ~ s(AC, bs = "cr", k = 10), 
                data = fit_df
                )
summary(fit_MIMS)
...
rm(Y, X, Y_resid_FPC1, subj_id_fc)
Y <- fit_df$MIMS
X <- fit_df$AC
Y_resid_FPC1 <- fit_df$MIMS_resid_FPC1
subj_id_fc <- fit_df$subj_id_F

t1 <- Sys.time()
fit_MIMS_gamm <- gamm4::gamm4(Y ~ s(X, bs = "cr", k = 10), 
                      random = ~ Y_resid_FPC1 | subj_id_fc
                      )
t2 <- Sys.time()
t2 - t1
