...
path_tmp <- paste0(here::here(), "/results/2021-04-28-mapping_threshold_pred_error.rds")
...
# ------------------------------------------------------------------------------
# ------------------------------------------------------------------------------
# DATA PROPORTION OF BEING ABOVE THE THRESHOLD
...
thresh_vec_A <- dat_fitted %>% filter(AC == AC_thresh) %>% unlist()
thresh_vec_B <- dat_fitted %>% filter(AC == (AC_thresh + 1)) %>% unlist()
thresh_vec   <- (thresh_vec_A + thresh_vec_B)/2
names(thresh_vec) <- gsub("_fitted", "", names(thresh_vec))
thresh_vec
...
check_df <- 
  ...
  select(all_of(c("AC_pred", names_levels))) %>%
  summarise_all(mean) %>%
  mutate(AC_thresh = AC_thresh, .before = everything()) %>% 
  as.data.frame()
check_df

df_out <- rbind(df_out, check_df); rm(check_df); dim(df_out)
...
# save results
path_tmp <- paste0(here::here(), "/results/2021-04-28-mapping_threshold_pred_proportion.rds")
saveRDS(df_out, path_tmp)
rm(df_out)
