...
masterfile_path <- paste0(here::here(), "/data_processed/2021-05-06-measures_masterfile_winsorized.rds")
...
plt_path <- paste0(here::here(), "/results_figures/2021-05-06-measures_corr_plot_few_scatterplot.png")
...
for (i in 1 : length(corr_subj_id_sub_vec)){ # i <- 1
  ...
  plt <- 
    ...
    labs(title = paste0("ID=", subj_id_i), 
         y = "", x = "Time") + 
    theme(plot.title = element_text(size = 10)) + 
    facet_grid(name_fct ~ ., scales = "free", switch="both") + 
...
plt <- do.call("arrangeGrob", c(plt_list, ncol = 3)) 
plt_path <- paste0(here::here(), "/results_figures/2021-05-06-measures_corr_plot_few_ts.png")
ggsave(filename = plt_path, plot = plt, width = 8, height = 8) 
