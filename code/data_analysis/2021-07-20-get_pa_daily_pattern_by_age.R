...
#' Estimate 24-hour median activity counts per minute for each age group. 
...
# get age summary
age_vec <- dat_acc %>% select(subj_id, age) %>% distinct() %>% pull(age)
quantile(age_vec, probs = seq(0.1, 0.9, by = 0.1))
# 10% 20% 30% 40% 50% 60% 70% 80% 90% 
#  50  58  64  69  72  75  79  82  86 
sort(age_vec)[round(seq(0.1, 0.9, by = 0.1) * length(age_vec))]
